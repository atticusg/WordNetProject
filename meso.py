    ...
    for synset in list(wn.all_synsets('n')):
        if hyp:
            for synset2 in synset.hyponyms() + synset.instance_hyponyms():
                G1.add_edges([synsetToId[synset], synsetToId[synset2]])
            ...
            for synset in wn.synsets(lemma_name, "n"):
                for synset2 in synset.hyponyms() + synset.instance_hyponyms():
                    for lemma_name2 in synset2.lemma_names():
                        ...
                        G1.add_edges([lemmaToId[lemma_name], lemmaToId[lemma_name2]])
...
G1, id2, synset2, _,_,_ = generate_meaning_graph(True, False, False)
print ("yes")
#print(G1.GetNodes())
#G2, id2, synset2, _,_,_ = generate_word_graph(False, True, False)
#G3, id2, synset2, _,_,_ = generate_word_graph(False, False, True)

partition = louvain.find_partition(G1, louvain.ModularityVertexPartition);
ig.plot(partition)
